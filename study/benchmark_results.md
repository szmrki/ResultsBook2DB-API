...
最終更新: 2026-08-11 / 状態: **経路1〜3 完了**（経路4 DuckDB のみ未測定）
...
最適な分割数（§6-6 の探索結果）で比較しても、差は残る。

| | Q1 | Q2 | Q3 | Q4 |
|---|---:|---:|---:|---:|
| PostgreSQL | 0.023 | 0.036 | 0.128 | 0.025 |
| Spark（最適分割時） | 0.149 | 0.162 | 0.546 | 0.107 |
| PG が速い倍率 | 6.5× | 4.5× | 4.3× | 4.3× |

クエリが重くなるほど差は縮まる（Q1 6.5× → Q3 4.3×）が、**逆転はしていない**。

**経路3（Parquet）でも覆らなかった。** 最も差が縮まった Q3 でも PostgreSQL が約1.9倍速い。
列指向の効果は「重いクエリで JOIN を消せるとき」に限られ、
この規模・このワークロードでは PostgreSQL を上回る根拠が得られていない。

ただし留保が2つ。

1. **経路4（DuckDB）を測っていない。** Spark は分散前提のオーバーヘッドを常に払う。
   単一マシン特化の DuckDB なら結果が変わりうる（`olap_analytics_plan.md` の本命は C 案）。
