...
最終更新: 2026-08-09 / 状態: **Q1 のみ完了**（Q2〜Q4 と経路3・4 は未測定）
...
**8.4倍の差**がついたが、これは演習書 §6-2 で予告した通りの結果であり異常ではない。
...
**「PostgreSQL は 157万行程度なら十分速い」という仮説が、Q1 に関しては裏付けられた。**
...
— は正しかった。**0.023秒は体感で待ち時間がなく、これを速くする動機は薄い。**

ただし**この結論はまだ早い**。理由は3つ。

1. **経路3（Parquet）を測っていない。** 列指向の効果はこれを測らないと分からない。
   Q1 の PG vs Spark+JDBC は「転送方式の比較」であって「行指向 vs 列指向の比較」ではない。
2. **Q1 は列指向が最も有利なはずのクエリ**（2列だけ読む単純集計）。それでも
   キャッシュ全ヒットのため差が出ていない。Q3（4段 JOIN）など重いクエリで傾向が変わる可能性。
3. **単発クエリしか測っていない。** analytics の実運用は「全大会横断で集計を何度も回す」形。
   キャッシュに載らない規模・パターンでは結果が変わりうる。

→ **判断は Q2〜Q4 と経路3・4 の測定後に行う。**

---

## 次にやること

- [ ] Q2（`shot_order` 別 GROUP BY）— `Exchange hashpartitioning` を確認
